...
class cheAdapter(private val context: Context, private val subjects: ArrayList<SubjectFireBase>) : RecyclerView.Adapter<cheAdapter.cheViewHolder>() {

    class cheViewHolder( view : View) : RecyclerView.ViewHolder(view){
        var textView : TextView = itemView.findViewById(R.id.subjectItem)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): cheViewHolder {
        val layoutAdapter = LayoutInflater.from(parent.context).inflate(R.layout.subject_item,parent,false)
        return cheViewHolder(layoutAdapter)
    }
    ...
    override fun onBindViewHolder(holder: cheViewHolder, position: Int) {
